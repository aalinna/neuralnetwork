...
class NeuralNetworkTester(neuralNetwork: NeuralNetwork) extends PeekPokeTester(neuralNetwork) {
  ...
  (0 until MNIST.num_hidden).foreach { neuron =>
    (0 until MNIST.num_inputs).foreach { axon =>
      poke(neuralNetwork.io.weight(0).valid, false)
    }
  }

  (0 until MNIST.num_outputs).foreach { neuron =>
    (0 until MNIST.num_hidden).foreach { axon =>
  ...
  (0 until MNIST.num_hidden).foreach { neuron =>
    (0 until MNIST.num_inputs).foreach { axon =>
      ...
      poke(neuralNetwork.io.weight(0).bits.weight, MNIST.weights_InputToHidden(neuron)(axon).toInt)
  ...
  (0 until MNIST.num_outputs).foreach { neuron =>
    (0 until MNIST.num_hidden).foreach { axon =>
      ...
      poke(neuralNetwork.io.weight(1).bits.weight, MNIST.weights_HiddenToOutput(neuron)(axon).toInt.abs)
  ...
  (0 until MNIST.num_inputs).foreach { axon =>
    ...
    poke(neuralNetwork.io.in.bits.data, 2)
...
object NeuralNetworkTester extends App {
  chisel3.iotesters.Driver(() => new NeuralNetwork()) { neuralNetwork => new NeuralNetworkTester(neuralNetwork) }
}
